...
def get_date_and_lineage_info_v2(input_path=None, int_weeks =None, identifier=None) -> pd.DataFrame:
    # i gave the inputs defaults of None, some where not being used but these should be used
    # file = return_latest_file(input_path, identifier)
    if input_path is None:
        print('please input the full path to the file')
        input_path = f'{input()}'
        # todo this is a terrible implementation that i have just put in for myself, it does need fixed, change to
        #     read from the config
    df = pd.read_csv(input_path,
                     usecols=["central_sample_id", "collection_date", "lineage"])
...
def generate_week_counts(input_df, int_weeks: int, date_column_name: str) -> [int, int]:
    ...
    # changed to count lineage rather than usher lineage, can check to see
...
def get_results_merged(results_df, week_counts_collapsed_masked) -> pd.DataFrame:
    ...
    results_merged = results_df.filter(
        regex='^(?!.*_DROP)').drop_duplicates()
    collapsing = week_counts_collapsed_masked[
        ['lineage', 'lineage_clean', 'collapsed_alias']].drop_duplicates().rename(
...
def generate_lineage_prevalence(file_path=None, save_path=os.getcwd()) -> None:
    ...
    # todo df_results could probably just be a read the file that is lineage, collection date, ID
